...
if (!nv_function_exists('nv_block_voting')) {
    ...
    {
        global $nv_Cache, $db, $site_mods, $global_config, $nv_Lang, $module_config;
        ...
        return $tpl->fetch('global.voting.tpl');

        echo '<pre><code>';
        echo htmlspecialchars(print_r($block_theme . 'xxx', true));
        echo htmlspecialchars(print_r($dir, true));
        die('</code></pre>');

        //$tpl->setTemplateDir(get_block_tpl_dir('global.banners.tpl', $block_config['module']));

        $block_theme = get_tpl_dir([$global_config['module_theme'], $global_config['site_theme']], 'default', '/modules/' . $site_mods[$module]['module_file'] . '/global.voting.tpl');
        ...
        $xtpl = new XTemplate('global.voting.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
